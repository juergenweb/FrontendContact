...
* Uploaded-files mail placeholder: identical filenames are now de-duplicated so the same file is no longer listed twice
* Uploaded-files mail placeholder: each filename now shows its file size in parentheses, e.g. "photo.jpg (2 MB)"
* Mail body: all empty custom field values now show a dash "-" instead of being left blank, not only the uploaded-files field as before
* File upload extension restriction redesigned: no longer read from the PW field's own `extensions` property (this value is only a suggested default shown in the admin UI and is not reliably persisted). The restriction now comes from FrontendForms' own global "Allowed mime types" setting, optionally overridden per field with `$field->setRule('allowedFileExt', [...])`
* saveEmail(): `$p->save('fcontact_files')` is now called once after the upload loop instead of once per uploaded file
* sendEmail(): removed a redundant `??` fallback on `$senderName` that could never be triggered

**Changes in tests**

* SecurityIntegrationTest: the check that the "Uploaded files" row is hidden when file upload is disabled now also checks for the German label variant ("Hochgeladene Dateien"), so the test does not silently pass for the wrong reason when the site language is German
